...
export function computeSupplierStats(cmds) {
  ...
  cmds.forEach(c => {
    ...
    if (c.datCde) {
      const dt = c.datCde instanceof Date ? c.datCde : new Date(c.datCde);
      if (!isNaN(dt) && (!map[name].dateMax || dt > map[name].dateMax)) {
        map[name].dateMax = dt;
      }
    }
    if (c.dateAffichage) {
      const dt = c.dateAffichage instanceof Date ? c.dateAffichage : new Date(c.dateAffichage);
      if (!isNaN(dt) && (!map[name].dateMax || dt > map[name].dateMax)) {
        map[name].dateMax = dt;
      }
    }
...
export { formatMontant, daysBetween };// --- Helpers ---
...
export function computeArticleStats(bcs) {
  ...
  // === Référentiel prix : un article = une ligne, avec toutes ses commandes ===
  const referentiel = Object.values(priceMap).map(a => {
    const sorted = [...a.entries].sort((x, y) => new Date(x.date) - new Date(y.date));
    const pus = sorted.map(e => e.pu).filter(p => p > 0);
    const fournisseurs = [...new Set(sorted.map(e => e.fournisseur))];
    const objets = [...new Set(sorted.map(e => e.objet).filter(Boolean))];
    const derniereDate = sorted.length > 0 ? sorted[sorted.length - 1].date : null;
    return {
      code: a.code,
      label: a.label,
      entries: sorted,
      nbCommandes: sorted.length,
      puActuel: pus.length > 0 ? pus[pus.length - 1] : 0,
      puMin: pus.length > 0 ? Math.min(...pus) : 0,
      puMax: pus.length > 0 ? Math.max(...pus) : 0,
      fournisseurs,
      objets,
      derniereDate,
    };
  ...
  return { priceEvolution, surstockCandidates, prixComparaison, prixAberrants, referentiel };
}
